const App = {
    audioCtx: null, analyser: null, buf: new Float32Array(4096), isPaused: false,
    pitchHistory: [], maxHistory: 150, smoothingWindow: [], smoothingSize: 10, currentCenterMidi: 60,
    droneOscs: [], droneGain: null, droneActive: false, selectedDrone: "C", isMetroOn: false, tempo: 120,
    refA4: 440, threshold: 0.015, chromatic: ['C','C#','D','D#','E','F','F#','G','G#','A','A#','B'],

    init() { this.setupNav(); this.bindEvents(); window.addEventListener('resize', () => this.resizeCanvas()); },
    bindEvents() {
        ...
        document.getElementById('metro-toggle').onclick = (e) => {
            this.isMetroOn = !this.isMetroOn; e.target.innerText = this.isMetroOn ? 'Stop' : 'Start';
    ...
    async start() {
        try {
            ...
            this.analyser.fftSize = 4096; source.connect(this.analyser);
    ...
    startDrone() {
        ...
        const filter = this.audioCtx.createBiquadFilter(); filter.type = "lowpass"; filter.frequency.value = 450;
        const root = this.getFreq(this.selectedDrone); const harmonics = [0.5, 1, 1.5, 2]; 
        harmonics.forEach((ratio, i) => {
            const osc = this.audioCtx.createOscillator(); osc.type = "sawtooth"; osc.frequency.value = root * ratio;
            const oscGain = this.audioCtx.createGain();
            oscGain.gain.value = (i === 1) ? 0.7 : (i === 0) ? 0.6 : (i === 2) ? 0.3 : 0.15;
            osc.connect(oscGain); oscGain.connect(filter); this.droneOscs.push(osc);
        });
    ...
    updateDrone() {
        if(this.droneOscs.length > 0) {
            const root = this.getFreq(this.selectedDrone); const ratios = [0.5, 1, 1.5, 2];
    ...
    drawHistogram() {
        const canvas = document.getElementById('history-canvas'); if(!canvas) return;
        const ctx = canvas.getContext('2d'); const w = canvas.width, h = canvas.height;
        ctx.clearRect(0,0,w,h);
        const range = 36; const minY = this.currentCenterMidi - 18, maxY = this.currentCenterMidi + 18;
        for (let m = Math.floor(minY); m <= Math.ceil(maxY); m++) {
            const y = h - ((m-minY)/range)*h; const isC = m%12 === 0;
            ctx.strokeStyle = isC ? '#334155' : '#1e293b'; ctx.lineWidth = isC ? 2 : 1;
            ctx.beginPath(); ctx.moveTo(0,y); ctx.lineTo(w,y); ctx.stroke();
            ctx.fillStyle = isC ? '#94a3b8' : '#475569'; ctx.font = '10px monospace';
            ctx.fillText(this.chromatic[((m % 12) + 12) % 12] + (Math.floor(m / 12) - 1), 12, y - 5);
        }
        if (this.pitchHistory.length < 2) return;
        ctx.lineWidth = 4; ctx.lineJoin = 'round'; ctx.shadowBlur = 12; ctx.shadowColor = '#0ea5e9'; ctx.beginPath();
        const step = w / this.maxHistory;
        this.pitchHistory.forEach((f, i) => {
            const m = 12 * Math.log2(f/440) + 69; const x = i * step, y = h - ((m-minY)/range)*h;
    ...
    loop() {
        if(!this.isPaused) {
            ...
            if(raw !== -1 && raw < 2500) {
                this.smoothingWindow.push(raw); if(this.smoothingWindow.length > this.smoothingSize) this.smoothingWindow.shift();
                const freq = [...this.smoothingWindow].sort((a,b)=>a-b)[Math.floor(this.smoothingWindow.length/2)];
                ...
                this.pitchHistory.push(freq); if(this.pitchHistory.length > this.maxHistory) this.pitchHistory.shift();
                this.drawHistogram();
            }
        }
    ...
    resizeCanvas() {
        const c = document.getElementById('history-canvas'); if(c) { c.width = c.clientWidth; c.height = c.clientHeight; }
    },
    setupNav() {
